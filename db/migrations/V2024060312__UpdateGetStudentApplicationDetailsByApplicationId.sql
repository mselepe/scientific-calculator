...
SELECT TOP 1
    ...
    (
        ...
        AND statusId = (SELECT statusId FROM applicationStatus WHERE status = 'In Review')
    ) AS commencementDate,
...
LEFT JOIN (
        SELECT ish.applicationId,(SELECT status FROM invoiceStatus WHERE invoiceStatus.statusId=ish.statusId) as inv_status, ish.statusId, ish.createdAt AS invCreatedAt
...
GROUP BY
    students.studentId, CONCAT(students.name, ' ', students.surname),latest_inv.inv_status ,applicationStatus.status ,applicationStatusHistory.statusId, amount, contactNumber, universityApplications.degreeName, students.streetAddress, students.suburb, students.city, students.postalCode, universityApplications.yearOfFunding, students.idDocumentName, studentApplications.academicTranscriptBlobName, studentApplications.matricCertificateBlobName, studentApplications.financialStatementBlobName, universityApplications.dateOfApplication, email, universities.universityName, faculties.facultyName, students.idNumber, yearOfStudy, universityApplications.applicationId, declinedReasonsHistory.reason,declinedReasonsHistory.motivation
ORDER BY
    yearOfStudy DESC;
