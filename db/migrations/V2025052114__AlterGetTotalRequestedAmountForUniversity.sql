...
WHERE university.universityName = @universityName
    ...
    AND ast.status IN ('Awaiting student response', 'In Review', 'Awaiting executive approval', 'Awaiting finance approval', 'Awaiting fund distribution', 'Contract')
    AND (
    ihs.statusId IS NULL OR ihs.statusId IN (
      SELECT t_ist.statusId
      FROM invoiceStatus t_ist
      WHERE t_ist.status IN ('Pending', 'Awaiting executive approval', 'Awaiting finance approval', 'Invalid')
    )
    );
END;
