...
SELECT TOP 1
    ...
    CAST(MAX(applicationStatusHistory.createdAt) AT TIME ZONE 'UTC' AT TIME ZONE 'South Africa Standard Time' AS DATETIME) AS lastStatusChangeDate,
    CASE 
        WHEN applicationStatusHistory.statusId <> (SELECT statusId FROM applicationStatus WHERE status = 'Approved') THEN 'Pending'
        ELSE COALESCE(
            (
                SELECT CONVERT(VARCHAR(50), CAST(MAX(applicationStatusHistory.createdAt) AT TIME ZONE 'UTC' AT TIME ZONE 'South Africa Standard Time' AS DATETIME)) 
    ...
    ISNULL(CAST(universityApplications.dateOfApplication AT TIME ZONE 'UTC' AT TIME ZONE 'South Africa Standard Time' AS DATETIME), CAST(GETDATE() AT TIME ZONE 'UTC' AT TIME ZONE 'South Africa Standard Time' AS DATETIME)) AS applicationDate,
    TRY_CAST(CONCAT(
        CASE 
