...
SELECT TOP 1
      ...
      FORMAT(MAX(applicationStatusHistory.createdAt) AT TIME ZONE 'UTC' AT TIME ZONE 'South Africa Standard Time', 'MMM dd, yyyy hh:mm tt') AS lastStatusChangeDate,
       CASE
         WHEN NOT EXISTS (
           ...
           WHERE ash.applicationId = universityApplications.applicationId
             AND ash.statusId = (
             SELECT statusId FROM applicationStatus WHERE status = 'Approved'
           )
         ) THEN 'Pending'
         ELSE (
           SELECT FORMAT(
                    MAX(ash.createdAt) AT TIME ZONE 'UTC' AT TIME ZONE 'South Africa Standard Time',
       ...
       studentApplications.confirmHonors AS confirmHonors,
       universityApplications.degreeName AS degree,
       universityApplications.degreeDuration AS degreeDuration,
...
GROUP BY
  students.studentId, students.name, students.surname, latest_inv.inv_status, applicationStatus.status, applicationStatusHistory.statusId, amount,
  contactNumber, universityApplications.degreeName,universityApplications.degreeDuration, students.streetAddress, students.suburb, students.city, students.postalCode,
  ...
  studentApplications.matricCertificateBlobName, studentApplications.financialStatementBlobName,studentApplications.confirmHonors, universityApplications.dateOfApplication,
