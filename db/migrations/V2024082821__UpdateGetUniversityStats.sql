/****** Object:  StoredProcedure [dbo].[GetUniversityStats]    Script Date: 2024/08/29 08:46:31 ******/
SET ANSI_NULLS ON
GO
...
ApplicationCounts AS (
    SELECT 
        ...
        SUM(CASE WHEN applicationStatus IN ('Draft','In Review','Awaiting student response','Approved','Email Failed') AND (invoiceStatusId IS NULL OR invoiceStatusId != 3) THEN 1 ELSE 0 END) AS applications
    FROM 
        UniqueApplications
    GROUP BY
        universityName
