...
GO
CREATE OR ALTER PROCEDURE [dbo].[renewApplication]
(
    @amount MONEY,
...
BEGIN
    ...
    BEGIN TRY
	  ...
	  SET @latestAcademicTranscipt =(SELECT docBlobName FROM (
        ...
        ROW_NUMBER() OVER(PARTITION BY createdAt ORDER BY universitySemesterId DESC) AS rn
        FROM
        academicTranscriptsHistory
       ) t
       WHERE t.rn = 1 AND  t.studentApplicationId = @prevApplicationId)
