IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetEventInvitees]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetEventInvitees]
GO

CREATE PROCEDURE [dbo].[GetEventInvitees]
...
BEGIN
	...
	SELECT (
		...
		INNER JOIN latestLocation AS ll ON ll.rsvpId = r.rsvpId AND ll.rn_location = 1
		INNER JOIN rsvpResponses AS rr ON lr.responseId = rr.responseId
		INNER JOIN eventLocations AS el ON ll.eventLocationId = el.eventLocationId
