...
BEGIN
    ...
    LatestNudges AS (
		SELECT
			createdAt,
			applicationId,
			ROW_NUMBER() OVER (PARTITION BY nudge.applicationId ORDER BY nudge.createdAt DESC)AS nrn
			FROM dbo.nudgeHistory
			AS nudge
			)
    ...
    SELECT 
        ...
        (
            SELECT
                ...
                nH.createdAt AS lastEmailSentTime
            ...
            LEFT JOIN LatestNudges nH ON nh.applicationId = universityApplication.applicationId AND nH.nrn=1
            LEFT JOIN 
                (
                    SELECT
                        ish.applicationId,
            ...
            WHERE 
				...
				AND  (@bursaryType IS NULL OR universityApplication.bursaryTypeId = (SELECT bursaryTypeId FROM bursaryTypes WHERE bursaryType = @bursaryType))
    ...
    FROM 
        (
            ...
            INNER JOIN 
                dbo.[applicationStatus] AS applicationStatus ON applicationStatusHistory.statusId = applicationStatus.statusId
			LEFT JOIN bursaryTypes bt ON bt.bursaryTypeId = universityApplication.bursaryTypeId 
