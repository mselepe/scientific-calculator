/****** Object:  StoredProcedure [dbo].[GetUniversityStats]    Script Date: 2024/08/16 12:48:25 ******/
SET ANSI_NULLS ON
GO
...
ApplicationCounts AS (
    SELECT 
        ...
        SUM(CASE WHEN applicationStatus NOT IN ('Rejected','Removed', 'Draft', 'Contract failed') AND (invoiceStatusId IS NULL OR invoiceStatusId != 3) THEN 1 ELSE 0 END) AS applications
    FROM 
        UniqueApplications
    GROUP BY
        universityName
