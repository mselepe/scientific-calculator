IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[GetStudentApplicationDetailsByApplicationId]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[GetStudentApplicationDetailsByApplicationId]
GO

CREATE PROCEDURE GetStudentApplicationDetailsByApplicationId
    @applicationId INT,
    @status NVARCHAR(50)
AS
BEGIN
SELECT TOP 1
    ...
    applicationStatusHistory.statusId,
    ...
    CASE 
        WHEN applicationStatusHistory.statusId <> 3 THEN 'Pending'
        ELSE COALESCE(
            (
    ...
    universityApplications.dateOfApplication AS applicationDate,
    DATEFROMPARTS(
        CASE 
...
INNER JOIN universityDepartments ON universityApplications.universityDepartmentId = universityDepartments.universityDepartmentId
INNER JOIN universities ON universityDepartments.universityId = universities.universityId
INNER JOIN faculties ON universityDepartments.facultyId = faculties.facultyId
INNER JOIN (
    SELECT 
        applicationId, 
...
INNER JOIN applicationStatusHistory ON applicationStatusHistory.applicationId = latestStatus.applicationId AND applicationStatusHistory.createdAt = latestStatus.latestCreatedAt
INNER JOIN studentApplications ON applicationStatusHistory.applicationId = studentApplications.applicationId
WHERE
    applicationStatusHistory.statusId = (SELECT statusId FROM applicationStatus WHERE status = @status)
	and applicationStatusHistory.applicationId=@applicationId
	
GROUP BY
    students.studentId, CONCAT(students.name, ' ', students.surname), applicationStatusHistory.statusId, amount, contactNumber, universityApplications.degreeName, students.streetAddress, students.suburb, students.city, students.postalCode, students.idDocumentName, studentApplications.academicTranscriptBlobName, studentApplications.matricCertificateBlobName, studentApplications.financialStatementBlobName, universityApplications.dateOfApplication, email, universities.universityName, faculties.facultyName, students.idNumber, yearOfStudy, universityApplications.applicationId
ORDER BY
    yearOfStudy DESC;
