...
BEGIN
    ...
    BEGIN
        INSERT INTO @applicationStatuses (status) 
        VALUES ('Draft'), ('In Review'), ('Awaiting student response'), ('Approved'), ('Email Failed'), ('Awaiting executive approval'), ('Awaiting finance approval');
    END

    DECLARE @invoiceStatuses TABLE (statusId INT);
    IF @rank = 'senior_admin'
    BEGIN
        INSERT INTO @invoiceStatuses (statusId) 
        VALUES (2),(3);  
    END
    ELSE IF @rank = 'admin_officer'
    BEGIN
    ...
    ;WITH UniqueApplications AS (
        ...
        LEFT JOIN 
            @invoiceStatuses AS invStat ON ish.statusId = invStat.statusId 
        WHERE 
    ...
    SUM(CASE 
        WHEN applicationStatus = 'Approved' 
             AND invoiceStatusId = 3 
        THEN 1 ELSE 0 END) AS activeBursaries,
    
    SUM(CASE 
        WHEN @rank = 'assistant_admin' 
            AND (invoiceStatusId IS NULL OR invoiceStatusId != 3) 
        THEN 1
        WHEN @rank != 'assistant_admin'
            AND (invoiceStatusId =2 or (invoiceStatusId IS NULL and (applicationStatus = 'Awaiting executive response' or applicationStatus = 'Awaiting finance approval')))
        THEN 1 
