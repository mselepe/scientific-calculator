IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[CountLatestApplicationData]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[CountLatestApplicationData]
GO

CREATE PROCEDURE CountLatestApplicationData
    @userId NVARCHAR(255),
    @status NVARCHAR(50),
...
BEGIN
    ...
    SELECT @resultCount = COUNT(*) FROM #TempApplicationData;

    DROP TABLE #TempApplicationData;

    SELECT @resultCount AS numberOfApplications;
END;
