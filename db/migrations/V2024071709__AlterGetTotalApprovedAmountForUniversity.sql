...
BEGIN
	...
	INNER JOIN 
	(
	SELECT MAX(createdAt) as lsv, applicationId, statusId
	FROM invoiceStatusHistory 
	GROUP BY applicationId,statusId
	...
	AND latestInv.statusId=(SELECT statusId FROM invoiceStatus WHERE status='Approved')
