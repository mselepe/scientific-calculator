...
BEGIN
    ...
    BEGIN
        ...
        VALUES ('Awaiting finance approval'), ('Approved'),('Onboarded');
    END
    ELSE IF @rank = 'admin_officer'
    BEGIN
        INSERT INTO @applicationStatuses (status) 
        VALUES ('Approved'), ('Awaiting executive approval'), ('Awaiting finance approval'),('Onboarded');
    END
    ELSE
    BEGIN
        INSERT INTO @applicationStatuses (status) 
		...
		,('Onboarded'),('Completing_Internal_Assessment'),('Conducting_Interview')
		;
    END
    ...
    SELECT
		...
		INNER JOIN bursaryTypes bt ON bt.bursaryTypeId = u.bursaryTypeId
        INNER JOIN (
            SELECT 
                applicationId, 
        ...
        WHERE
			(@bursaryType IS NULL OR bt.bursaryType = @bursaryType)
			AND
...
GROUP BY
    ...
    TotalAllocations AS (
		...
		INNER JOIN bursaryTypes bt ON bt.bursaryTypeId = a.bursaryTypeId 
        WHERE 
