...
BEGIN
	;WITH CurrentYearApplications AS (
	 ...
	 latestApplicationHistory AS (
		SELECT applicationId, createdAt,statusId, ROW_NUMBER() OVER(PARTITION BY applicationStatusHistory.applicationId ORDER BY applicationStatusHistory.createdAt DESC, applicationStatusHistory.applicationStatusHistoryId DESC) AS rn
    ...
    INNER JOIN 
        latestApplicationHistory AS lah ON lah.applicationId = gtu.applicationId AND lah.rn = 1
    INNER JOIN 
        applicationStatus AS a ON lah.statusId = a.statusId
    INNER JOIN 
        universityApplications AS up ON up.applicationId = lah.applicationId
    LEFT JOIN (
        SELECT
            ish.applicationId,
